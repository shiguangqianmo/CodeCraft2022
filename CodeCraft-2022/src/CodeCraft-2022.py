...
def pre_calc():
    ...
    for t in range(0, T):       # 遍历时间
        aval_band_t = Aval_band[t, :]  # t时刻所有节点的aval_band N
        for n in range(0, N):   # 遍历节点
            aval_band_t[n] = 0
            client_index = np.where(QoS[n, :] > 0)[0]   # 所连客户索引 x
            client_aval_band = np.multiply(np.repeat(np.expand_dims(aval_band_t, axis=0), client_index.shape[0], axis=0), np.transpose(QoS[:, client_index]))   # 每个关联客户的aval_band, x * N
            ...
            aval_space = np.minimum(reallocation_cap, sch_n)    # 各客户的可优化空间
    ...
    return obj_node_index, Up_limit[obj_node_index], t_index, Aval_band


def optimize2():
    _iter = 0
    while(_iter < max_iter):
        cur_node_index, up_limit, t_index, Aval_band = pre_calc()     # 当前考虑的节点索引，优化上限，所有节点的95带宽
        # 将t_index时刻，cur_node的带宽压至上限
        client_index = np.where(QoS[cur_node_index, :] > 0)[0]  # 当前节点关联的客户索引
        for t in t_index:
            sch_t = Schedule[t, :, :]   # 该时刻的分配方案  M，N
            optim_space_t = np.sum(sch_t[:, cur_node_index]) - up_limit     # t时刻这个节点可优化空间（当前负载-上限）
            ...
            aval_band_t = Aval_band[t, :]  # t时刻所有节点的aval_band
            aval_band_t[cur_node_index] = 0  # 本身节点置为0
            for m in client_index:
                aval_band_m = np.multiply(aval_band_t, np.transpose(QoS[:, m]))     # 该时刻m客户所连节点的aval_band，N
                aval_space_m = np.minimum(np.sum(aval_band_m), Schedule[t, m, cur_node_index])  # 该客户的可分配空间
                if aval_space_m > 0:     # 如果该客户还可以进行重新分配
                    if aval_space_m > optim_space_t:    # 这个客户很强，不需要考虑下一个客户
                        m_sch = aval_band_m / np.sum(aval_band_m) * optim_space_t  # 让有aval_band的节点承担
                        node_index = np.where(m_sch > 0)[0]
                        m_sch = np.floor(m_sch)
                        m_sch[node_index[-1]] = optim_space_t - np.sum(m_sch[node_index[0: -1]])
                        num = int(m_sch[node_index[-1]] - aval_band_m[node_index[-1]])
                        if num > 0:
                            m_sch[node_index[0: num]] += 1
                            m_sch[node_index[-1]] = aval_band_m[node_index[-1]]
                        # aval_band_t -= m_sch
                        # optim_space_t -= np.sum(m_sch)
                        Schedule[t, m, :] += m_sch
                        Schedule[t, m, cur_node_index] -= np.sum(m_sch)
                        break
                    else:
                        m_sch = aval_band_m / np.sum(aval_band_m) * aval_space_m
                        node_index = np.where(m_sch > 0)[0]
                        m_sch = np.floor(m_sch)
                        m_sch[node_index[-1]] = aval_space_m - np.sum(m_sch[node_index[0: -1]])
                        num = int(m_sch[node_index[-1]] - aval_band_m[node_index[-1]])
                        if num > 0:
                            m_sch[node_index[0: num]] += 1
                            m_sch[node_index[-1]] = aval_band_m[node_index[-1]]
